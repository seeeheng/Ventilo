...
public class TimelineAdHocRecyclerAdapter extends RecyclerView.Adapter<TimelineViewHolder> {
...
//    private View mInnerContainerView;
    ...
    public TimelineAdHocRecyclerAdapter(Context context, List<TaskModel> timelineAdHocListItems) {
        this.mContext = context;
        mTimelineAdHocListItems = timelineAdHocListItems;
    }
...
//                viewGroup, false);
    ...
    public void onBindViewHolder(TimelineViewHolder itemViewHolder, final int i) {
        ...
        TaskModel taskModel = mTimelineAdHocListItems.get(i);

        // -------------------- Set Phase Number, Task Title and Description --------------------
        if (EAdHocTaskPriority.HIGH.toString().equalsIgnoreCase(taskModel.getAdHocTaskPriority())) {
            itemViewHolder.getLinearLayoutHeaderAdHocHighPriorityIcon().setVisibility(View.VISIBLE);
        } else {
        ...
        itemViewHolder.getTvHeaderPhaseNumber().setTextColor(
                ContextCompat.getColor(mContext, R.color.primary_highlight_cyan));

        String phase = taskModel.getPhaseNo();
        ...
        for (int j = 0; j < assignedToStrArray.length; j++) {
            ...
            // If true, it means completedDateTimeStrArray does NOT contain empty string
            ...
            // -------------------- Get Final Status and Inner Container Statuses --------------------
        ...
        if (isAllCompleted) {
            ...
            itemViewHolder.getTvFinalCompletionTime().setText(StringUtil.EMPTY_STRING);
            itemViewHolder.getImgTimelineLineStatus().setImageDrawable(ResourcesCompat.
                    getDrawable(mContext.getResources(), R.drawable.icon_new_unselected, null));
        } else {
            itemViewHolder.getTvFinalCompletionTime().setText(StringUtil.EMPTY_STRING);
    ...
    public void setTimelineListItems(List<TaskModel> timelinePhaseListItems) {
        mTimelineAdHocListItems = timelinePhaseListItems;
        notifyDataSetChanged();
    }

    @Override
    public int getItemCount() {
        return mTimelineAdHocListItems.size();
    }
}
